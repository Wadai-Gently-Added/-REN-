...
function createCircleBlock(units, type, shape, rotationMode, cx, cy, radius, boxSize, shiftX, shiftY) {
  ...
  const isTrace = type === 'grayFull' || type === 'grayVert' || type === 'grayNone';
  const txtColor = isModel ? '#333' : (isTrace ? 'rgba(180,180,180,0.85)' : 'transparent');
  ...
  for (let index=0; index<placeCount; index++) {
    ...
    if ((isModel || isTrace) && !isSpace) {
      h += `<text x="0" y="0" dy="0.1em" font-size="${fontSize}" font-family="${window.__practiceFontFamily || "'Yu Mincho', serif"}" font-weight="bold" text-anchor="middle" dominant-baseline="middle" fill="${txtColor}">${unit}</text>`;
    }
    h += `</g>`;
  }
  h += `</g>`;
...
function renderCircle() {
  ...
  // 以前は Math.min(18, ...) で強制的に小さくしていたのが
  // 「隙間があるのに少ない個数しか出ない／文字が小さい」原因。
  // 直径と文字サイズをきちんと連動させるため上限を緩和。
  const rawBox = diameter * BOX_RATIO;
  const boxSize = Math.max(8, Math.min(42, Math.round(rawBox * 10) / 10));
  ...
  // 直径に応じてパディングと最小ギャップをスケール
  const pad = Math.max(2.0, Math.min(5.0, diameter * 0.05));
  const minGap = Math.max(1.8, Math.min(4.5, diameter * 0.045));
  const blockW = bounds.width + pad, blockH = bounds.height + pad;
  const shiftX = bounds.centerX, shiftY = bounds.centerY;

  const pageW = 210, pageH = 297;
  // 大きい直径のときは外側余白を多めにして端に寄りすぎないようにする
  const baseMargin = diameter >= 90 ? 12 : (diameter >= 70 ? 10 : 8);
  ...
  const maxPerPage = Math.max(1, cols * rows);

  // 複数ページ対応：必要枚数を計算
  const pageCount = Math.max(1, Math.ceil(useCount / maxPerPage));

  const isVerticalPack = (packDir === 'vertical');
  const host = byId('circlePagesHost') || byId('circlePage')?.parentElement;
  const firstPage = byId('circlePage');

  // 既存の追加ページを掃除
  if (host) {
    Array.from(host.querySelectorAll('.page-container.extra-page')).forEach(el => el.remove());
  }

  function buildOnePage(pageTypes, pageIndex) {
    let actualCols, actualRows;
    const n = pageTypes.length;
    if (isVerticalPack) {
      actualRows = Math.min(rows, n);
      actualCols = Math.min(cols, Math.ceil(n / Math.max(1, actualRows)));
    } else {
      actualCols = Math.min(cols, n);
      actualRows = Math.min(rows, Math.ceil(n / Math.max(1, actualCols)));
    }
    ...
    let h = '<rect width="210" height="297" fill="#fff"/>';
    ...
    h += `<text x="105" y="11" font-size="5.1" font-family="${window.__practiceFontFamily || "'Yu Mincho', serif"}" font-weight="bold" text-anchor="middle" fill="#555">${shapeNames[shape] || shape}美文字練習帳（直径${diameter}）${pageLabel}</text>`;
    ...
    for (let i = 0; i < n; i++) {
      ...
      h += createCircleBlock(units, pageTypes[i], shape, rotationMode, cx, cy, radius, boxSize, shiftX, shiftY);
    }
    return h;
  }

  // 1ページ目
  const firstTypes = types.slice(0, maxPerPage);
  const svg = byId('mainSvg');
  if (svg) svg.innerHTML = buildOnePage(firstTypes, 0);

  // 2ページ目以降
  for (let p = 1; p < pageCount; p++) {
    const slice = types.slice(p * maxPerPage, (p + 1) * maxPerPage);
    const wrap = document.createElement('div');
    wrap.className = 'page-container extra-page';
    const s = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    s.setAttribute('class', 'workspace-svg');
    s.setAttribute('viewBox', '0 0 210 297');
    s.setAttribute('preserveAspectRatio', 'xMidYMid meet');
    s.innerHTML = buildOnePage(slice, p);
    wrap.appendChild(s);
